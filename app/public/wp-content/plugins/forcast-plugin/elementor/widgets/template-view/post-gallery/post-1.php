<div class="bytf__post-slider-wrapper">
    <div class="bytf__post__gallery-for">
        <?php
        if ( $query->have_posts() ) {
    ...
    <div class="bytf__post__gallery-nav">
        <?php
        if ( $query->have_posts() ) {
        while ( $query->have_posts() ) {
            $query->the_post();
            $idd = get_the_ID();
            $pfythumb_size = 'full';
            $title2 = wp_trim_words( get_the_title(), $settings['title_length_th'], '' );
            if(get_post_meta($idd, 'empath_post_format_meta', true)) {
                $post_video_meta = get_post_meta($idd, 'empath_post_format_meta', true);
            } else {
                $post_video_meta = array();
        ...
        <div class="bytf__post-overlay-item">
            ...
            <div class="bytf__content">
                <?php if(function_exists('empath_category_badge_two')){empath_category_badge_two();}?>
                <h4 class="bytf_post_title"><?php echo esc_html($title2);?></h4>
                <?php 
                    if($settings['excerpt_hide'] === 'yes'){ ?>
                <p>
                    <?php echo wp_trim_words( get_the_excerpt(), $settings['excerpt_length'], '' ); ?>
