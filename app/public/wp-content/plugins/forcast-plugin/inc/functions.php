 ...
 * Ajax Loadmore Data
 */
function empath_post_ajax_loading_cb(){   
...
<div class="bytf__post-grid-rating-item-two pfy-grid-item ajax__post-item d-flex align-items-center">
    ...
    <div class="bytf__content">
        ...
        <h2 class="bytf_post_title"><a aria-label="name" href="<?php the_permalink();?>"><?php echo esc_html($title);?></a></h2> 
        <?php 
                    ...
                    <?php if('yes' === $settings['cmnt_hide']):?>
                        ...
                        <?php echo esc_attr(get_comments_number());?> <?php
                            if(get_comments_number() == 1 ){
                                esc_html_e( 'Comment', 'empath-plugin' );
                            }else{
                                esc_html_e( 'Comments', 'empath-plugin' );
                            }                                
                        ?>
                        </li>
                    <?php endif;?>
    ...
    <div class="bytf__feature-image">
        ...
        <?php 
                    if('video' == get_post_format( $idd )){ ?>
                        <span class="post_frmt_icon"><i class="far fa-play"></i></span>
                    <?php 
                    }elseif('gallery' == get_post_format( $idd )){ ?>
                    ...
                    <?php } elseif('audio' == get_post_format( $idd )){?>
                        <span class="post_frmt_icon">
...
add_action( 'wp_ajax_nopriv_empath_post_ajax_loading', 'empath_post_ajax_loading_cb' );
add_action( 'wp_ajax_empath_post_ajax_loading', 'empath_post_ajax_loading_cb' );


/**
 * Ajax Loadmore Data
 */
function empath_post_ajax_loading_cb_two(){   
    $settings =  $_POST['ajax_json_data'];
    if ( get_query_var('paged') ) {
        $paged = get_query_var('paged');
    } else if ( get_query_var('page') ) {
...
<div class="bytf__post-list-item-two style__two pfy-grid-item d-flex align-items-center">               
    <div class="bytf__feature-image">
        <a aria-label="name" href="<?php the_permalink();?>">
            <?php the_post_thumbnail( 'full' );?>
        </a>
...
add_action( 'wp_ajax_nopriv_empath_post_ajax_loading_2', 'empath_post_ajax_loading_cb_two' );
add_action( 'wp_ajax_empath_post_ajax_loading_2', 'empath_post_ajax_loading_cb_two' );
 ...
 * @return void
 */
function empath_single_share() {

    $facebook_enable = cs_get_option( 'facebook_enable' );	
    $twitter_enable = cs_get_option( 'twitter_enable' );	
    $linkedin_enable = cs_get_option( 'linkedin_enable' );	
    $pinterest_enable = cs_get_option( 'pinterest_enable' );	
    $whatsapp_enable = cs_get_option( 'whatsapp_enable' );	
    $email_enable = cs_get_option( 'email_enable' );	
    $print_enable = cs_get_option( 'print_enable' );	

    $permalink = get_permalink( get_the_ID() );
    $title     = get_the_title();
    if($facebook_enable == true || $twitter_enable == true || $linkedin_enable == true || $pinterest_enable == true || $whatsapp_enable == true || $email_enable == true):
?>
<div class="social-box d-flex justify-content-between">
    <div class="share-wrap">
        <span class="share-title"><?php esc_html_e( 'Share This Article:', 'empath-tools' ); ?></span>

    <?php if($facebook_enable == true):?>
        <a aria-label="name" class="fb" onClick="window.open('http://www.facebook.com/sharer.php?u=<?php echo esc_url( $permalink ); ?>','Facebook','width=600,height=300,left='+(screen.availWidth/2-300)+',top='+(screen.availHeight/2-150)+''); return false;" href="http://www.facebook.com/sharer.php?u=<?php echo esc_url( $permalink ); ?>"><i class="fab fa-facebook-f"></i></a>
    <?php endif;?>

    <?php if($twitter_enable == true):?>
        <a aria-label="name" class="tw" onClick="window.open('http://twitter.com/share?url=<?php echo esc_url( $permalink ); ?>&amp;text=<?php echo esc_attr( $title ); ?>','Twitter share','width=600,height=300,left='+(screen.availWidth/2-300)+',top='+(screen.availHeight/2-150)+''); return false;" href="http://twitter.com/share?url=<?php echo esc_url( $permalink ); ?>&amp;text=<?php echo str_replace( " ", "%20", $title ); ?>"><i class="fab fa-twitter"></i></a>
    <?php endif;?>

    <?php if($linkedin_enable == true):?>
    ...
    <?php if($whatsapp_enable == true):?>
        <a aria-label="name" class="wh" onClick="window.open('https://api.whatsapp.com/send?text=<?php echo esc_url( $permalink ); ?>&amp;text=<?php echo esc_attr( $title ); ?>','Linkedin share','width=600,height=300,left='+(screen.availWidth/2-300)+',top='+(screen.availHeight/2-150)+''); return false;" href="https://api.whatsapp.com/send?text=<?php echo esc_url( $permalink ); ?>&amp;text=<?php echo str_replace( " ", "%20", $title ); ?>"><i class="fab fa-whatsapp"></i></a>
    <?php endif;?>

    <?php if($pinterest_enable == true):?>
    ...
    <?php if($email_enable == true):?>
            <a aria-label="name" class="em" href='mailto:?subject="<?php echo esc_attr( $title ); ?>&<?php echo esc_url( $permalink ); ?>"'><i class="fal fa-envelope"></i></a>
        <?php endif;?>
        <?php if($print_enable == true):?>
